...
data BlockchainConfig = BlockchainConfig -- TODO
...
-- TODO: serialization
-- instance ToJSON UnverifiedBlockchain where
-- instance FromJSON UnverifiedBlockchain where

data BlockchainVerificationException = BlockchainVerificationException AddBlockException
...
verify (UnverifiedBlockchain config (UnverifiedBlockchainNode genesisBlock nodes)) =
    Either.mapLeft BlockchainVerificationException $ Foldable.foldrM addBlock blockchainHead blocks
  where
    -- TODO: verify genesisBlock conforms to config specs (initial difficulty, initial reward, etc.)
